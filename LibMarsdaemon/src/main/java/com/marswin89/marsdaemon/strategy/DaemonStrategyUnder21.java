...
public class DaemonStrategyUnder21 implements IDaemonStrategy{
	...
	public void onDaemonAssistantCreate(Context context, DaemonConfigurations configs) {
		...
		Log.v("ke_test","onDaemonAssistantCreate() android.os.Process.killProcess");
...
//		}
	...
	public void onDaemonDead() {
		Log.v("ke_test","onDaemonDead()");
		mAlarmManager.setRepeating(AlarmManager.ELAPSED_REALTIME, SystemClock.elapsedRealtime(), 100, mPendingIntent);
	//	mAlarmManager.set(AlarmManager.ELAPSED_REALTIME,SystemClock.elapsedRealtime(),mPendingIntent);
	//	android.os.Process.killProcess(android.os.Process.myPid());
		if (Build.VERSION.SDK_INT>=19) {
			android.os.Process.killProcess(android.os.Process.myPid());
		}else {
		...
		Log.v("ke_test","onDaemonDead() android.os.Process.killProcess");
	}
