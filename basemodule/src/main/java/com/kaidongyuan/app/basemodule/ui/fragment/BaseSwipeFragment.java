...
public class BaseSwipeFragment extends BaseLifecyclePrintFragment {
    ...
    public void onAttach(Activity activity) {
        ...
        swipeLayout = new SwipeLayout(getMContext());
    }
    ...
    public void onDetach() {
        ...
        if (swipeFinished) {
//            finish();
//            getFragmentManager().beginTransaction().setCustomAnimations(0, R.anim.slide_out_right).remove(this).commit();
        } else {
...
//            overridePendingTransition(0, R.anim.slide_out_right);
    ...
    class SwipeLayout extends FrameLayout {
        ...
        public void replaceLayer(Fragment fragment) {
            touchSlop = (int) (touchSlopDP * fragment.getResources().getDisplayMetrics().density);
            sideWidth = (int) (sideWidthInDP * fragment.getResources().getDisplayMetrics().density);
            mFragment = fragment;
            screenWidth = getScreenWidth(fragment.getActivity());
            setClickable(true);
            backgroundLayer = new View(fragment.getActivity());
            backgroundLayer.setBackgroundColor(layerColor);
            final ViewGroup root = (ViewGroup) fragment.getActivity().getWindow().getDecorView();
...
//                        getFragmentManager().popBackStack();
                        finish();
//                        getFragmentManager().beginTransaction().setCustomAnimations(0, R.anim.slide_out_right).remove(BaseSwipeFragment.this).commit();
                    }
                }
